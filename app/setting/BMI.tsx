...
const BMIIndicator = ({ bmi }: { bmi: number }) => {
  ...
  return (
    <View className="items-center mt-6">
      <Text className="text-white font-bold text-lg mb-2">Your BMI: {bmi.toFixed(1)}</Text>

      <View className="w-full h-5 bg-gray-700 rounded-full overflow-hidden">
        <View
          style={{
      ...
      <Text className="text-gray-300 text-sm mt-2">{category}</Text>
    </View>
  );
};
  ...
  const renderCounter = (label: string, value: number, setValue: (v: number) => void, unit: string, min: number, max: number) => (
    <View className="items-center px-5 py-6 rounded-3xl bg-gray-800/60 border border-gray-700 mx-6">
      <Text className="text-gray-300 text-sm mb-1">{label}</Text>
      <View className="flex-row items-center justify-between w-full">
        ...
        <Pressable
          className="p-3 bg-indigo-600/80 rounded-full"
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-900 px-6">
      ...
      <View className="mt-auto mb-10 px-4">
        <TouchableOpacity
          className="h-14 bg-white rounded-full items-center justify-center"
          onPress={handleNext}
        >
